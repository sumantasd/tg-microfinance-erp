...
<x-ui.card class="shadow-sm border-0 p-4">
    <form action="{{ route('admin.product-purchase.update', $productPurchase->id) }}" method="POST">
        ...
        <div class="row g-3">
            ...
            <div class="col-md-4">
                <label class="form-label fw-bold small">Registered Supplier</label>
                <select name="supplier_id" id="supplier_select" class="form-select @error('supplier_id') is-invalid @enderror" onchange="if(this.value){ document.getElementById('supplier_name_input').value = this.options[this.selectedIndex].getAttribute('data-name'); }">
                    ...
                    @foreach($suppliers as $sup)
                        <option value="{{ $sup->id }}" data-name="{{ $sup->supplier_name }}" {{ old('supplier_id', $productPurchase->supplier_id) == $sup->id ? 'selected' : '' }}>
            ...
            <div class="col-md-4">
                <label class="form-label fw-bold small">Supplier Name (Display) <span class="text-danger">*</span></label>
                <input type="text" name="supplier_name" id="supplier_name_input" class="form-control @error('supplier_name') is-invalid @enderror" value="{{ old('supplier_name', $productPurchase->supplier_name) }}" required>
            ...
            <div class="col-12" id="purchaseItemsContainer">
                @foreach($productPurchase->items as $idx => $item)
                    ...
                    @endphp
                    <div class="row g-2 mb-3 purchase-item-row border rounded p-2.5 bg-light-subtle align-items-center">
                        <div class="col-md-2">
                            <label class="form-label small fw-bold text-muted mb-1">Product Category <span class="text-danger">*</span></label>
                            <select name="items[{{ $idx }}][category_id]" class="form-select form-select-sm category-select" required>
                        ...
                        <div class="col-md-2">
                            ...
                            <select name="items[{{ $idx }}][brand_id]" class="form-select form-select-sm brand-select" required>
                        ...
                        <div class="col-md-3 position-relative">
                            ...
                            <input type="hidden" name="items[{{ $idx }}][product_id]" class="product-id-input" value="{{ $item->product_id }}" required>
                            <div class="input-group input-group-sm">
                                <input type="text" class="form-control form-control-sm product-search-input" value="{{ $item->product_name_snapshot }} (SKU: {{ $item->product_sku_snapshot }})" placeholder="Type to search product..." autocomplete="off">
                        ...
                        <div class="col-md-1">
                            ...
                            <input type="number" name="items[{{ $idx }}][quantity]" class="form-control form-control-sm item-qty" value="{{ $item->quantity }}" min="1" required>
                        </div>
                        <div class="col-md-2">
                            <label class="form-label small fw-bold text-muted mb-1">Unit Cost Price (₹)</label>
                            <input type="number" step="0.01" name="items[{{ $idx }}][unit_purchase_cost]" class="form-control form-control-sm unit-cost-input" value="{{ $item->unit_purchase_cost }}">
                        </div>
                        <div class="col-md-1">
                            <label class="form-label small fw-bold text-muted mb-1">GST Tax %</label>
                            <input type="number" step="0.01" name="items[{{ $idx }}][tax_rate]" class="form-control form-control-sm tax-rate-input" value="{{ $item->tax_rate }}">
                        </div>
                        <div class="col-md-1 d-flex align-items-end justify-content-center">
                            <button type="button" class="btn btn-sm btn-outline-danger w-100 btn-remove-row {{ count($productPurchase->items) <= 1 ? 'disabled' : '' }}" title="Remove line item">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        let rowIdx = {{ count($productPurchase->items) }};
