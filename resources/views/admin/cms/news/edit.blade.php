...
@section('title', 'Edit News Article - TG Microfinance ERP')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h4 class="fw-bold text-dark mb-1">Edit News Article</h4>
        <p class="text-muted small mb-0">Update article content, featured graphic image, and publication date.</p>
    </div>
...
<x-ui.card class="p-4 shadow-sm">
    <form action="{{ route('admin.cms.news.update', $news->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="row g-3">
            <div class="col-md-8">
                ...
                <input type="text" name="title" value="{{ old('title', $news->title) }}" class="form-control bg-light" required>
            </div>

            <div class="col-md-4">
                <label class="form-label small fw-bold text-secondary">URL Slug</label>
                <input type="text" name="slug" value="{{ old('slug', $news->slug) }}" class="form-control bg-light font-monospace">
            </div>

            <div class="col-md-4">
                <label class="form-label small fw-bold text-secondary">Published Date</label>
                <input type="date" name="published_date" value="{{ old('published_date', $news->published_date ? $news->published_date->format('Y-m-d') : date('Y-m-d')) }}" class="form-control bg-light">
            </div>
            ...
            <div class="col-md-4">
                ...
                <input type="number" name="sort_order" value="{{ old('sort_order', $news->sort_order) }}" class="form-control bg-light" required>
            </div>
            ...
            <div class="col-12">
                ...
                @if($news->featured_image_url)
                    <div class="mt-2 p-2 bg-light border rounded d-flex align-items-center gap-3">
                        <img src="{{ $news->featured_image_url }}" alt="Featured Image" class="rounded border" style="max-height: 60px;">
            ...
            <div class="col-12">
                ...
                <textarea name="short_description" rows="2" class="form-control bg-light">{{ old('short_description', $news->short_description) }}</textarea>
            </div>

            <div class="col-12">
                <label class="form-label small fw-bold text-secondary">Full Article Content (HTML allowed)</label>
                <textarea name="content" rows="8" class="form-control bg-light">{{ old('content', $news->content) }}</textarea>
            </div>

            <div class="col-12 pt-3 border-top d-flex gap-2">
                <button type="submit" class="btn btn-primary rounded-pill px-4 py-2 fw-bold shadow-sm">
                    <i class="bi bi-check-circle me-1"></i> Update News Article
                </button>
