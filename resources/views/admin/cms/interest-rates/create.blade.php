...
@section('title', 'Add Interest Rate Schedule Entry - TG Microfinance ERP')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h4 class="fw-bold text-dark mb-1">Add Rate Schedule Entry</h4>
        <p class="text-muted small mb-0">Configure product rates, calculation method, and processing fee structure.</p>
    </div>
...
<x-ui.card class="p-4 shadow-sm">
    <form action="{{ route('admin.cms.interest-rates.store') }}" method="POST">
        @csrf

        <div class="row g-3">
            <div class="col-md-6">
                <label class="form-label small fw-bold text-secondary">Product Name *</label>
                <input type="text" name="product_name" value="{{ old('product_name') }}" class="form-control bg-light" placeholder="e.g. Micro-Enterprise Loan" required>
            </div>
            ...
            <div class="col-md-3">
                ...
                <select name="product_type" class="form-select bg-light" required>
                    <option value="loan" {{ old('product_type') === 'loan' ? 'selected' : '' }}>Loan Product</option>
                    <option value="savings" {{ old('product_type') === 'savings' ? 'selected' : '' }}>Savings Scheme</option>
                </select>
            </div>
            ...
            <div class="col-md-3">
                ...
                <select name="interest_method" class="form-select bg-light" required>
                    ...
                    <option value="Reducing Balance" {{ old('interest_method', 'Reducing Balance') === 'Reducing Balance' ? 'selected' : '' }}>Reducing Balance</option>
                    <option value="Daily Reducing" {{ old('interest_method') === 'Daily Reducing' ? 'selected' : '' }}>Daily Reducing</option>
                </select>
            </div>

            <div class="col-md-4">
                <label class="form-label small fw-bold text-secondary">Interest Rate (P.A.) *</label>
                <input type="text" name="interest_rate" value="{{ old('interest_rate') }}" class="form-control bg-light" placeholder="e.g. 12.5% P.A." required>
            </div>

            <div class="col-md-4">
                <label class="form-label small fw-bold text-secondary">Min-Max Amount Range</label>
                <input type="text" name="amount_range" value="{{ old('amount_range') }}" class="form-control bg-light" placeholder="e.g. $500 – $5,000">
            </div>

            <div class="col-md-4">
                <label class="form-label small fw-bold text-secondary">Tenure Options</label>
                <input type="text" name="tenure_options" value="{{ old('tenure_options') }}" class="form-control bg-light" placeholder="e.g. 6 – 18 Months">
            </div>

            <div class="col-md-6">
                <label class="form-label small fw-bold text-secondary">Processing Fee (%)</label>
                <input type="text" name="processing_fee" value="{{ old('processing_fee') }}" class="form-control bg-light" placeholder="e.g. 1.0%">
            </div>
            ...
            <div class="col-12">
                ...
                <textarea name="description" rows="3" class="form-control bg-light" placeholder="Calculated on monthly reducing balance method...">{{ old('description') }}</textarea>
            </div>

            <div class="col-12 pt-3 border-top d-flex gap-2">
                <button type="submit" class="btn btn-primary rounded-pill px-4 py-2 fw-bold shadow-sm">
                    <i class="bi bi-check-circle me-1"></i> Save Rate Entry
                </button>
