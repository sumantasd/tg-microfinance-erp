...
@section('title', 'Add New Role' . ' - ' . config('app.name'))

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h4 class="fw-bold text-dark mb-1">Create New Role</h4>
        <p class="text-muted small mb-0">Define role title and configure the granular module permission matrix.</p>
    </div>
                        ...
                        <div class="d-flex flex-column gap-2">
                            @foreach($groupPermissions as $permission)
                                <div class="form-check">
                                    <input class="form-check-input perm-check perm-group-{{ $group }}" type="checkbox" name="permissions[]" value="{{ $permission->name }}" id="perm_{{ $permission->id }}">
